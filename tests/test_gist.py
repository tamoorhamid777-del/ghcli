...
MOCK_GISTS = [{"id": "abc123def456", "description": "My gist", "public": True, "files": {"test.py": {}}, "updated_at": "2026-01-01T00:00:00Z"}]
MOCK_GIST = {"id": "abc123def456", "description": "My gist", "public": True, "files": {"test.py": {"content": "print('hello')", "language": "Python"}}, "updated_at": "2026-01-01T00:00:00Z"}

def make_client(get_val=None, post_val=None):
    client = MagicMock()
    client.get.return_value = get_val
    client.post.return_value = post_val or {"id": "abc123", "html_url": "https://gist.github.com/abc123"}
    return client
...
def test_gist_create():
    ...
    with patch("ghcli.commands.gist.GitHubClient") as MockClient:
        ...
        result = runner.invoke(gist, ["create", "hello.py", "print('hello')", "--description", "Test"])
    assert result.exit_code == 0
