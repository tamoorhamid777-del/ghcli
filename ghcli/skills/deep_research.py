...
DEFAULT_TIMEOUT = 15
DEFAULT_HEADERS = {
    "User-Agent": "ghcli/2.0.0 (research-skill; +https://github.com/ghcli)"
}
...
class GitHubSearchAdapter(SourceAdapter):
    ...
    def search(self, query: str, limit: int = 5, kind: str = "repositories", **kwargs) -> List[Dict[str, Any]]:
        """
...
class URLExtractAdapter(SourceAdapter):
    ...
    def search(self, query: str, limit: int = 1, selector: str = "", **kwargs) -> List[Dict[str, Any]]:
        ...
        for step in plan.steps:
            ...
            if adapter is None:
                results.append(ResearchResult(
                    source=step.source, query=step.query, items=[],
                    error=f"Unknown source '{step.source}'. Available: {list(self._adapters)}",
                ))
                continue
            if verbose:
                console.print(f"[dim]  → Querying [bold]{step.source}[/bold]: {step.query!r}…[/dim]")
            t0 = time.time()
            try:
                items = adapter.search(step.query, limit=step.limit, **step.extra)
                results.append(ResearchResult(
                    source=step.source, query=step.query,
                    items=items, elapsed=time.time() - t0,
                ))
    ...
    def print_report(self, report: ResearchReport) -> None:
        ...
        for result in report.results:
            ...
            table = Table(
                ...
                box=box.SIMPLE, header_style="bold cyan", show_lines=True,
            )
